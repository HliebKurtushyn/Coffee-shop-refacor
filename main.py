...
# Робить цю функцію доступною в усіх шаблонах
...
def register():
    if request.method == 'POST':
        ...
        if len(password) < 8:
            ...
            return render_template('sign-in-up/register.html',
...
def login():
    ...
    return render_template('sign-in-up/login.html', 
                        csrf_token=session["csrf_token"], 
                        current_year=datetime.now().year)
...
def position(name):
    if request.method == 'POST':
        ...
        with Session() as db_session:
            ...
            flash(f"Додано {new_basket_item.quantity} шт. {new_basket_item.menu.name} до кошика")
...
def basket():
    with Session() as db_session:
        ...
        if request.method == 'POST':
            ...
            db_session.commit()
            flash(f"Видалено {basket_item.menu.name} з кошика")
    ...
    with Session() as db_session:
        if request.method == 'POST':
            ...
            if not basket_items:
                ...
                flash(f"Замовлення оформлено! Загальна сума: {total_price}₴")
...
def add_position():
    ...
    previous_url = session.get('previous_url') or url_for('admin_panel')
...
def add_offer():
    ...
    with Session() as db_session:
        ...
        if request.method == "POST":
            ...
            db_session.add(new_position)
            db_session.commit()
            flash('Пропозицію додано успішно!')
